...
const Modal_Crear_Infraestructura = ({
  ...
  const handleNuevaInfraestructura = async () => {
    try {
      const data = await crearInfraestructura(nombre);
      setTipoInfraestructura([...tipoInfraestructura, data]);
      Swal.fire({
        position: "center",
        icon: "success",
      ...
      console.log(
        "Hubo error en el Componente Modal_Crear_Infraestructura",
        e
      );
    }
  };
    ...
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 z-10 flex items-center justify-center">
        <div className="max-w-lg w-11/12 max-h-[90vh] bg-white shadow-2xl rounded-2xl p-5">
          ...
          <h2 className="text-3xl font-bold text-center">Crear Nueva Infraestructura</h2>

          <form
            className="mt-5"
            onSubmit={(e) => {
              e.preventDefault();
              handleNuevaInfraestructura();
            }}
          >
            <div>
              <h3 className="font-semibold mt-2">Nombre de Infraestructura</h3>
              <input
...
export default Modal_Crear_Infraestructura;
