...
import { actualizarInfraestructura, crearTurno } from "../../../api/UnidadesEducativas";
import { DataContext } from "../../../context/DataProvider";

const Modal_Editar_Infraestructura = ({
  open,
  onClose,
  idInfraestructura,
  ...
  const [nombre, setNombre] = useState(nombreInfraestructura);

  const actualizarTurno = async () => {
    try {
      let id = idInfraestructura;
    
      const data = await actualizarInfraestructura(id, nombre ); // Asegúrate de que la API espera un objeto
      setDatoInfraestructura( datoInfraestructura.map( dato => (dato.id === id) ? data : dato ) );
      Swal.fire({
        position: "center",
        icon: "success",
...
export default Modal_Editar_Infraestructura;