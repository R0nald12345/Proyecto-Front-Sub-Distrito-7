import { useState, useEffect } from 'react';
import axios from 'axios';
import ListaVistaDesayunoApoyoSocial from './ListaVistaDesayunoApoyoSocial';

const FormVistaDesayunoApoyoSocial = ({ id}) => {
    const [datosGeneralDesayuno, setDatosGeneralDesayuno] = useState([]);
    const [datosDesayuno, setDatosDesayuno] = useState([]);
    const [datosDatoGeneralApoyoSocial, setDatoGeneralApoyoSocial] = useState([]);
    const [datoApoyoSocial, setDatoApoyoSocial] = useState([]);
    const [tipoApoyoSeleccionado, setTipoApoyoSeleccionado] = useState('Apoyo Social');

    const [tipoApoyo, setTipoApoyo] = useState([]);

    useEffect(() => {
        setTipoApoyo([
            { id: 1, nombre: 'Apoyo Social' },
            { id: 2, nombre: 'Desayuno' },
        ]);
    }, []);

    useEffect(() => {
        const fetchingDatosDesayuno = async () => {
            try {
                const baseUrl = import.meta.env.VITE_BASE_URL;
                const url = baseUrl + `/desayunos`;
                const datosDesayuno = await axios.get(url);
                setDatosGeneralDesayuno(datosDesayuno.data);

                const datosFiltrados = datosDesayuno.data.filter(
                    (dato) => dato.idUnidadeducativa == id
                );
                setDatosDesayuno(datosFiltrados);
            } catch (error) {
                console.log("Error al cargar los datos Desayuno" + error);
            }
        };
        fetchingDatosDesayuno();
    }, [id]);

    useEffect(() => {
        const fetchingDatoApoyoSocial = async () => {
            try {
                const baseUrl = import.meta.env.VITE_BASE_URL;
                const url = baseUrl + `/apoyosocial`;
                const datosApoyoSocial = await axios.get(url);

                setDatoGeneralApoyoSocial(datosApoyoSocial.data);

                const datosFiltrados = datosApoyoSocial.data.filter(
                    (dato) => dato.idUnidadeducativa == id
                );
                setDatoApoyoSocial(datosFiltrados);

            } catch (error) {
                console.log("Error al cargar los datos Apoyo Social" + error);
            }

        }
        fetchingDatoApoyoSocial();

    }, [id])

    const handleTipoApoyoChange = (event) => {
        setTipoApoyoSeleccionado(event.target.value);
    };

    return (
        <>
            <select
                className='py-1 rounded-xl pl-3 w-full border-gray-400 border-2 bg-gray-200 mb-3'
                onChange={handleTipoApoyoChange}
                value={tipoApoyoSeleccionado}
            >
                {tipoApoyo.map(option => (
                    <option value={option.nombre} key={option.id}>{option.nombre}</option>
                ))}
            </select>

            <section className='border border-black/50 rounded-lg px-5 py-3'>
                <section className='w-full'>
                    <ul className='grid grid-cols-11 bg-white gap-5 mb-3 rounded-xl shadow-lg'>
                        <li className=" font-semibold text-start col-span-3 px-3 py-2 ">Fecha</li>
                        <li className=" font-semibold text-start col-span-3 px-3 py-2 ">NombreEntrega</li>
                        <li className=" font-semibold text-start col-span-2 px-3 py-2 ">Nombre</li>
                        <li className=" font-semibold text-center col-span-3 px-2 py-2">Cantidad</li>
                    </ul>
                </section>

                <section className="w-full max-h-96 overflow-y-auto" style={{ "scrollbarWidth": "none" }}>
                    {tipoApoyoSeleccionado === 'Apoyo Social' &&
                        datoApoyoSocial.map((element) => (
                            <ListaVistaDesayunoApoyoSocial
                                key={element.id}
                                fecha={element.fecha}
                                nombreEntrega={element.nombreEntrega}
                                nombre={element.nombre}
                                cantidad={element.cantidad}
                            />
                        ))
                    }

                    {tipoApoyoSeleccionado === 'Desayuno' &&
                        datosDesayuno.map((element) => (
                            <ListaVistaDesayunoApoyoSocial
                                key={element.id}
                                fecha={element.fecha}
                                nombreEntrega={element.nombreEntrega}
                                nombre={element.nombre}
                                cantidad={element.cantidad}
                            />
                        ))
                    }
                </section>
            </section>
        </>
    )
}

export default FormVistaDesayunoApoyoSocial;
